...
var page_from= queryString("page_from");//点击入口
...
//    });
	...
	$('#rechargeButton').click(function() {
	   Callback.doRecharge($('#money_order').val(),$('#have_id_card').val(),$('#have_bank_card').val(),$('#name').val(),$('#idCard').val(),$('#bankCard').val(),page_from);
	});
