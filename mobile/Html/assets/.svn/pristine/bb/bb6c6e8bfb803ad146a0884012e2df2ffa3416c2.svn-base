...
$(function(){
	 ...
	 $('#sendMsgButton').click(function() {
			if(interval==60){
					//发送短信验证码
					Callback.getRandomSms($("#mobile").val(),$("#pwd").val());
				}
	 ...
	 //注册
	  $('#registerButton').click(function() {
		  ...
		  if(!$('#regBox').is(':checked')) {
			   //alert($("#regBox").attr('checked'));
			   Callback.showToast("请同意《益大家注册协议》及《益大家服务条款》");
			   return;
			}
			Callback.doRegister($('#mobile').val(), $('#pwd').val(), $('#code').val());
		});


	 //注册协议
	 $('#registerProtocol').click(function() {
			 Callback.gotoWebViewPage("file:///android_asset/registerprotocol.html", "false");
		});

	  //服务条款
	 $('#serviceProtocol').click(function() {
			 Callback.gotoWebViewPage("file:///android_asset/serviceprotocol.html", "false");
		});
