...
var urlJson={base:'/weixin',wallet:'0',ydb:'1'}
...
function remoteMessage(result) {
	...
	var Validate={
		...
		_applyYdb:function(amount,blanceId,min_num,max_num){
			var amount=new Number($('#'+amount).val());
			var balance=new Number($('#'+blanceId).val());
			
			if(this._isnull(amount)){
				Sys.showToast("金额不能为空");
				return false;
			}else{
				var regu =/^[1-9][0-9]*(.[0-9]{1,2})?$/;
				var re = new RegExp(regu);
				if (re.test(amount)) {
				    if (!isNaN(amount) && amount<min_num){
				        Sys.showToast('投标金额不得少于'+min_num);
			            return false;
			        }else if(amount>max_num){
			        	Sys.showToast( "大于最大投资金额，最多可投" + max_num + "元");
			        }else{
			        	return true;
			        }
				}else{
